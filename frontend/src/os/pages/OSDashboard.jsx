    ...
    <div data-testid="os-dashboard" className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {summary?.revenue_visible !== false ? (
          ...
          <Card k={k} testId="metric-revenue-locked">
            <div className="flex items-start justify-between mb-3">
              <p className={`text-[10px] font-mono tracking-[0.24em] uppercase ${k.muted}`}>{t.dashboard.revenue}</p>
              <DollarSign className={`w-4 h-4 ${k.muted}`} />
            </div>
            <p className={`text-sm ${k.muted}`}>{lang === "ar" ? "🔒 محدود للمالية فقط" : "🔒 Finance role only"}</p>
          </Card>
        )}
